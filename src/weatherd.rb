...
# Handle color manipulation for colors
class WeatherColors
  def initialize(time, temperature, summary)
    @time = time
  ...
  #   5 |
  #   4 |
  def weather_to_colors
    colors = [base_color(@summary)] * 6

    tip = tip_color(@temperature)
    if tip
      colors[1] = hex_blend(colors[1], tip, 100) # Doing a full capture for now
      colors[3] = hex_blend(colors[3], tip, 100) # due to daytime wash-out of
      colors[5] = hex_blend(colors[5], tip, 100) # LED colors displayed.
    end
